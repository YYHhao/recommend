...
class MainWindow(QWidget):
    def __init__(self, user):
        ...
        self.user_df = pd.read_csv('recommend\\DoubanMovieRecommendation_code\\data\\douban_users.csv')
        self.user_df = self.user_df.iloc[:,[1,2,3]]
        self.movies_df = pd.read_csv('recommend\\DoubanMovieRecommendation_code\\data\\doubanMovies.csv', encoding='utf-8')
        ...
        f = open('recommend\\DoubanMovieRecommendation_code\\data\\users_info.csv', 'a+', encoding='utf-8')
        f.seek(0) # 文件指针指向开头
        for line in f:
